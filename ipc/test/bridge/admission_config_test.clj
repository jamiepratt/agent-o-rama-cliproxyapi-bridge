...
(defn scrape-value [observer metric]
  (let [text (:body (obs-test/request (:port observer) "/metrics" "GET"))]
    (Double/parseDouble (second (re-find (re-pattern (str "(?m)^" metric " ([0-9.]+)$")) text)))))
  ...
  (let [{:keys [client ipc module-name calls bounded-release]} ipc/*context*
    ...
    (try
      ...
      (let [queued (aor/agent-initiate client {:prompt "small-1" :call-id "small-1"})]
        ...
        (is (= 1.0 (metric "bridge_queue_depth")))
        ...
        (is (= 1.0 (- (metric "bridge_overloads_total") overloads)))
        (is (= 1 (- (count @calls) before)))
      ...
      (doseq [request [{:prompt "failure-shared" :call-id "shared-error" :model-name "spike-nonexistent-model"}
        ...
        (let [before (count @calls)
              coalesces (metric "bridge_coalescing_total")
              errors (metric "bridge_upstream_errors_total")
              timeouts (metric "bridge_upstream_timeouts_total")
          ...
          (let [duplicate (aor/agent-initiate client request)]
            ...
            (is (= 1.0 (- (metric "bridge_coalescing_total") coalesces)))
            (is (= 1 (- (count @calls) before)))
            (let [healthy (aor/agent-initiate client {:prompt "fixture"})]
              ...
              (is (ipc/eventually #(empty? (:active (rama/foreign-select-one ["admission"] state)))))
              (is (= 0.0 (metric "bridge_active_calls") (metric "bridge_queue_depth")))
              (is (= (double (count (filter #(= (:prompt request) (get-in % [:messages 0 :content])) (drop before @calls))))
                     (- (metric "bridge_upstream_errors_total") errors)))
              (when (:timeout? request)
                (is (= (- (metric "bridge_upstream_errors_total") errors)
                       (- (metric "bridge_upstream_timeouts_total") timeouts))))))))
